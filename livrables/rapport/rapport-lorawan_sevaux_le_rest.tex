...
Pour vérifier si \textit{PCROP} a bien été implémenté il faut brancher la carte au programme \textit{St-link Utility} et afficher les octets d'option pour voir les secteur sécuriser, comme on peut le voir sur la figure \ref{SecuSecteur}  
 
\begin{figure}[!h]
\centering
\includegraphics[scale=0.6]{img/eps/PCROPVerif.eps}
\caption{Capture d'écran du logiciel \textit{St-Link Utility} sur laquelle on peut voir Des secteur sécurisé et non sécurisé }
\label{SecuSecteur}
\end{figure}
\pagebreak
